...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # print('cpts with evidence:', cpts)`
        for var in eliminationOrder:
            currentFactorsList, inferenceFactor = joinFactorsByVariable(currentFactorsList, var)
            if len(inferenceFactor.unconditionedVariables()) > 1:
                singleMarginalizedJoint = eliminate(inferenceFactor, var)
                currentFactorsList.append(singleMarginalizedJoint)
        currentFactorsList = joinFactors(currentFactorsList) # not sure if necessary
        currentFactorsList = normalize(currentFactorsList) # check this later
