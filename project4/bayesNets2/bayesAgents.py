...
def fillObsCPT(bayesNet, gameState):
    ...
    def isAdjacentToHouse(house, pos):
        def isAdjacentTo(thisPosition, thatPosition):
            return (abs(thisPosition[0] - thatPosition[0]) == 1 and thisPosition[1] == thatPosition[1]) \
                or (abs(thisPosition[1] - thatPosition[1]) == 1 and thisPosition[0] == thatPosition[0]) \
                or (abs(thisPosition[0] - thatPosition[0]) == 1 and (abs(thisPosition[1] - thatPosition[1]) == 1))
        if house == TOP_LEFT_VAL:
            return isAdjacentTo(topLeftPos, pos)
        elif house == BOTTOM_LEFT_VAL:
            return isAdjacentTo(bottomLeftPos, pos)
        elif house == TOP_RIGHT_VAL:
            return isAdjacentTo(topRightPos, pos)
        elif house == BOTTOM_RIGHT_VAL:
            return isAdjacentTo(bottomRightPos, pos)
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                if isAdjacentToHouse(foodHouse, obsPos) and assignment[obsVar] == BLUE_OBS_VAL:
                    obsFactor.setProbability(assignment, 1 - PROB_FOOD_RED)
                elif isAdjacentToHouse(foodHouse, obsPos) and assignment[obsVar] == RED_OBS_VAL:
                    obsFactor.setProbability(assignment, PROB_FOOD_RED)
                elif isAdjacentToHouse(foodHouse, obsPos) and assignment[obsVar] == NO_OBS_VAL:
                    obsFactor.setProbability(assignment, 0)
                elif isAdjacentToHouse(ghostHouse, obsPos) and assignment[obsVar] == BLUE_OBS_VAL:
                    obsFactor.setProbability(assignment, 1 - PROB_GHOST_RED)
                elif isAdjacentToHouse(ghostHouse, obsPos) and assignment[obsVar] == RED_OBS_VAL:
                    obsFactor.setProbability(assignment, PROB_GHOST_RED)
                elif isAdjacentToHouse(ghostHouse, obsPos) and assignment[obsVar] == NO_OBS_VAL:
                    obsFactor.setProbability(assignment, 0)
    ...
    # for housePos in gameState.getPossibleHouses():
    #     for houseWall in gameState.getHouseWalls(housePos):
    #         for obsVal in OBS_VALS:
    #             for foodHouseVal in HOUSE_VALS:
    #                 for ghostHouseVal in HOUSE_VALS:
    #                     obsFactor = bn.Factor([assigment], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
    #                     obsFactor.setProbability({houseWall: obsVal, FOOD_HOUSE_VAR: foodHouseVal, GHOST_HOUSE_VAR: ghostHouseVal}, ___ )
    #                     bayesNet.setCPT(obsVar, obsFactor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    inferenceFactor = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    #print('inference factor:', inferenceFactor)
    assignments = inferenceFactor.getAllPossibleAssignmentDicts()

    maxProbability = -1
    maxAssignment = None
    for assignment in assignments:
        rowProbability = inferenceFactor.getProbability(assignment)
        if rowProbability > maxProbability:
            maxProbability = rowProbability
            maxAssignment = assignment
    #print('assignment:', maxAssignment)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        inferenceFactor = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        #print('assignment dicts:', inferenceFactor.getAllPossibleAssignmentDicts())
        leftQuery = {FOOD_HOUSE_VAR: TOP_LEFT_VAL, GHOST_HOUSE_VAR: TOP_RIGHT_VAL}
        rightQuery = {FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL}
        leftQuery.update(evidence)
        rightQuery.update(evidence)
        pFoodLeftGhostRight = inferenceFactor.getProbability(leftQuery)
        pFoodRightGhostLeft = inferenceFactor.getProbability(rightQuery)
        leftExpectedValue = pFoodLeftGhostRight * WON_GAME_REWARD + pFoodRightGhostLeft * GHOST_COLLISION_REWARD
        rightExpectedValue = pFoodRightGhostLeft * WON_GAME_REWARD + pFoodLeftGhostRight * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        #print('probs and outcomes:', probsAndOutcomes)
        for prob, newEvidence in probsAndOutcomes:
